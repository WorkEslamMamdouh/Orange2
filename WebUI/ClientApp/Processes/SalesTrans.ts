...
namespace SalesTrans {

    //System
    var sys: SystemTools = new SystemTools();
    var SysSession: SystemSession = GetSystemSession(Modules.SalesTrans);
    var compcode: Number;
    var Branch: Number;
    ...
    var OperSalesmanDetails: Array<IQ_GetOperationSalesman> = new Array<IQ_GetOperationSalesman>();
    var IQ_SalesOperTransferWithDetail: IQ_GetOPerationTransferWithDetail = new IQ_GetOPerationTransferWithDetail();
    var SearchDetails: Array<IQ_GetOperationTF> = new Array<IQ_GetOperationTF>();
    var OpSalsmanItem: IQ_GetOperationSalesmanItem = new IQ_GetOperationSalesmanItem;
    var ItemsListDetails: Array<IQ_GetOperationSalesmanItem> = new Array<IQ_GetOperationSalesmanItem>();
    ...
    var operationDetails: Array<IQ_GetOperation> = new Array<IQ_GetOperation>();
    var SalesmanDetails: Array<I_Sls_D_Salesman> = new Array<I_Sls_D_Salesman>();
    var SalesmanTODetails: Array<I_Sls_D_Salesman> = new Array<I_Sls_D_Salesman>();

    //Models
    var MasterDetailModel: OPerationSalesmanTransferWithDetail = new OPerationSalesmanTransferWithDetail();
    var HeaderModel: I_TR_OperationTF = new I_TR_OperationTF();
    var DetailModel: Array<I_TR_OperationTFDetail> = new Array<I_TR_OperationTFDetail>();
    var DetailSingleModel: I_TR_OperationTFDetail = new I_TR_OperationTFDetail();
    var DetailModelFiltered: Array<IQ_GetOperationTFDetail> = new Array<IQ_GetOperationTFDetail>();
    var IQTransferDetailModel: Array<IQ_GetTransferDetail> = new Array<IQ_GetTransferDetail>();
    var IQDetailSingleModel: IQ_GetTransferDetail = new IQ_GetTransferDetail();
    var SelectedModel: Array<IQ_GetOperationTF> = new Array<IQ_GetOperationTF>();
    ...
    var txtOPerationStatus: HTMLInputElement;

    //DropdownLists
    var ddlStatusFilter: HTMLSelectElement;
    var ddlSourceSalesman: HTMLSelectElement;
    var ddlToSalesman: HTMLSelectElement;

    var ddlSourceSalesmanAdd: HTMLSelectElement;
    var ddlToSalesmanAdd: HTMLSelectElement;
    ...
    var GlobalOPerationID: number = 0;
    var GlobalOPerationStatus: boolean = false;// true--> open && false----close 
    ...
    function InitalizeControls() {
        ...
        txtOPerationStatus = document.getElementById("txtOPerationStatus") as HTMLInputElement;

        //DropdownLists
        ddlStatusFilter = document.getElementById("ddlStatusFilter") as HTMLSelectElement;
        ddlSourceSalesman = document.getElementById("ddlSourceSalesman") as HTMLSelectElement;
        ddlToSalesman = document.getElementById("ddlToSalesman") as HTMLSelectElement;
        ddlSourceSalesmanAdd = document.getElementById("ddlSourceSalesmanAdd") as HTMLSelectElement;
        ddlToSalesmanAdd = document.getElementById("ddlToSalesmanAdd") as HTMLSelectElement;
    ...
    function ddlOPerationMaster_onchange() {
        ...
        if (ddlOPerationMaster.value != "null") {
            ...
            var opObj = operationDetails.filter(s => s.OperationID == opID);
            if (opObj[0].Status == 2) {
    ...
    function BindGridData() {
        ...
        Ajax.Callsync({
            ...
            success: (d) => {
                ...
                if (result.IsSuccess) {
                    IQ_SalesOperTransferWithDetail = new IQ_GetOPerationTransferWithDetail();
                    IQ_SalesOperTransferWithDetail = result.Response as IQ_GetOPerationTransferWithDetail;
                    for (let i = 0; i < IQ_SalesOperTransferWithDetail.IQ_GetOperationTF.length; i++) {
    ...
    function GridRowDoubleClick() {
        ...
        if (AfterInsertOrUpdateFlag == true) {
            SelectedModel = IQ_SalesOperTransferWithDetail.IQ_GetOperationTF.filter(x => x.OperationTFID == GlobalTransferID);
        ...
        if (SelectedModel.length > 0) {
            ...
            // Details
            DetailModelFiltered = new Array<IQ_GetOperationTFDetail>();

            DetailModelFiltered = IQ_SalesOperTransferWithDetail.IQ_GetOperationTFDetail.filter(s => s.OperationTFID == GlobalTransferID);

            for (let i = 0; i < DetailModelFiltered.length; i++) {
    ...
    function fillddlSourceSalesmanAdd(opeartionID: number) {
        Ajax.Callsync({
            type: "Get",
            ...
            success: (d) => {
                let result = d as BaseResponse;
                if (result.IsSuccess) {
                    OperSalesmanDetails = result.Response as Array<IQ_GetOperationSalesman>;
                    if (SysSession.CurrentEnvironment.ScreenLanguage == "en") {
    ...
    function fillddlToSalesmanAdd() {
        Ajax.Callsync({
            ...
            success: (d) => {
                let result = d as BaseResponse;
                if (result.IsSuccess) {
                    SalesmanTODetails = result.Response as Array<I_Sls_D_Salesman>;
    ...
    function fillddlOperation() {
        Ajax.Callsync({
            ...
            success: (d) => {
                let result = d as BaseResponse;
                if (result.IsSuccess) {
                    operationDetails = result.Response as Array<IQ_GetOperation>;
    ...
    function txtSearch_onKeyup() {
        if (txtSearch.value != "") {
            ...
            SearchDetails = IQ_SalesOperTransferWithDetail.IQ_GetOperationTF.filter(x => x.FromSls_NameA.toString().toLowerCase().search(search) >= 0 || x.FromSls_NameE.toString().toLowerCase().search(search) >= 0
                || x.IsSent_Desc.toLowerCase().search(search) >= 0 || x.ReceivedBy.toLowerCase().search(search) >= 0
                || x.ToSls_NameA.toLowerCase().search(search) >= 0 || x.Tosls_NameE.toLowerCase().search(search) >= 0
                || x.TrType_Desc.toString().search(search) >= 0 || x.Vnd_nameA.toString().search(search) >= 0
                || x.Vnd_NameE.toString().search(search) >= 0 || x.RefNO.toString().search(search) >= 0 || x.Tr_No.toString().search(search) >= 0);
    ...
    function BuildControls(cnt: number) {
        ...
        $('#btnSearchItems' + cnt).click(function (e) {
            ...
            sys.FindKey(Modules.SalesTrans, "btnSearchItems", "OperationID=" + opID + " and OnhandQty > 0  and SalesmanId= " + Salsman, () => {
                let id = SearchGrid.SearchDataGrid.SelectedKey
                var res = false;
                var opList = ItemsListDetails.filter(s => s.OperationItemID == id && s.SalesmanId == Salsman);
        ...
        $("#txtRecQty" + cnt).on('keyup', function () {
            ...
            var selectedItem = ItemsListDetails.filter(s => s.ItemID == ItemID);
    ...
    //---------------------------------------------- get By id  functions ----------------------------------------
    function GetAllOPerationItem(operationID: number) {
        Ajax.Callsync({
            type: "Get",
            ...
            success: (d) => {
                let result = d as BaseResponse;
                if (result.IsSuccess) {
                    ItemsListDetails = result.Response as Array<IQ_GetOperationSalesmanItem>;
                }
            }
        });
    }
    ...
    function Assign() {
        ...
        DetailModel = new Array<I_TR_OperationTFDetail>();
        ...
        if (chkApproved.checked == true) { HeaderModel.IsSent = true; } else { HeaderModel.IsSent = false; }
        ...
        Ajax.Callsync({
            ...
            success: (d) => {
                ...
                if (result.IsSuccess == true) {
                    let res = result.Response as I_TR_OperationTF;
    ...
    function Save(OperationTFID: number) {
        InitializeGrid();
        $("#div_hedr").removeClass("disabledDiv");
        $("#divShow").removeClass("disabledDiv");
        $("#divGridShow").removeClass("disabledDiv");
        ...
        SelectedModel = IQ_SalesOperTransferWithDetail.IQ_GetOperationTF.filter(x => x.OperationTFID == OperationTFID);
    ...
    export function PrintReport(OutType: number) {
        ...
        let FromBra = $('#ddlSourceSalesman').val();
        let ToBra = $('#ddlToSalesman').val();
