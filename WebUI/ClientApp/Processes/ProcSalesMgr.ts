...
namespace ProcSalesMgr {
    //system varables
    var SysSession: SystemSession = GetSystemSession(Modules.ProcSalesMgr);
    var compcode: Number;
    ...
    var operationDetails: Array<IQ_GetOperationSalesman> = new Array<IQ_GetOperationSalesman>();
  ...
  //  var btnPrint: HTMLInputElement;
    ...
    function fillddlSalesmanAdd(OperID: number) {
        Ajax.Callsync({
            type: "Get",
    ...
    function ddlOPerationMaster_onchange() {
        ...
        if (ddlOPerationMaster.value.trim() == "" || ddlOPerationMaster.value == "0") {
            ...
            var operObj = operationDetailsWithoutStatus.filter(s => s.OperationID == OperaID);
    ...
    function ddlSalesman_onchange() {
        ...
        if (ddlSalesman.value != "null") {
            ...
            ItemDetails = mainItemDetails.filter(s => s.SalesmanId == salID && s.OnhandQty > 0);
            AddNewRow();
        }
    ...
    function Validation_Grid(rowcount: number) {
        ...
        if ($("#txt_StatusFlag" + rowcount).val() == "d" || $("#txt_StatusFlag" + rowcount).val() == "m") {
            ...
            var Prc: number = Number($("#txtPrice" + rowcount).val());
            var Prc_WithVat: number = Number($("#txtUnitpriceWithVat" + rowcount).val());
    ...
    function btnAdd_onclick() {
        ...
        if (ddlOPerationMaster.value.trim() == "" || ddlOPerationMaster.value == "0") {
            ...
            if (chkOpenProcess.checked == false) {
                ...
                ItemDetails = new Array<IQ_GetOperationSalesmanItem>();
                $('#ddlItem').empty();
                //AddNewRow();
                chkActive.checked = true;
                chkActive.disabled = false;
    ...
    function Grid_RowDoubleClicked() {
        ...
        if (InvoiceStatisticsModel.length) {
            ...
            ItemDetails = mainItemDetails.filter(s => s.SalesmanId == salID);
    ...
    function BuildControls(cnt: number) {
        ...
        $("#txtPrice" + cnt).on('change', function () {
            ...
            let GetUnitprice: IGetunitprice = Get_PriceWithVAT(SalesPrice, VatPrc, false);
        ...
        $("#txtUnitpriceWithVat" + cnt).on('change', function () {
            ...
            let GetUnitprice: IGetunitprice = Get_PriceWithVAT(SalesPrice, VatPrc, true);
    ...
    function Assign() {
        ...
        if ($("#txtCardMoney").val() != "") InvoiceModel.CardAmount = Number($("#txtCardMoney").val());
        if ($("#txtCashMoney").val() != "") InvoiceModel.CashAmount = Number($("#txtCashMoney").val());
        if (ddlInvoiceCustomer.value != "null") {
            InvoiceModel.CustomerId = Number(ddlInvoiceCustomer.value);
            var custid = Number(ddlInvoiceCustomer.value);
            var custList = CustomerDetails.filter(s => s.CustomerId == custid)
    ...
    export function PrintReport(OutType: number) {
        ...
        if ($("#ddlOPerationMaster").val().trim() == "") { rp.OperationId = -1; }
        else { rp.OperationId = Number(OperaID); }
