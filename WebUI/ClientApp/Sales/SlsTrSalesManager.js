...
(function (SlsTrSalesManager) {
    ...
    function btnAdd_onclick() {
        ...
        if (SysSession.CurrentEnvironment.I_Control.RetailInvoicePaymentDef == 2) {
            ...
            chkActive.checked = SysSession.CurrentEnvironment.I_Control[0].IsRetailCashInvoiceDefAuth;
            if (compcode != 4) {
                filterCustomerDetails = CustomerDetails.filter(function (x) { return x.Isactive == true && x.IsCreditCustomer == true; });
                DocumentActions.FillCombowithdefult(filterCustomerDetails, ddlInvoiceCustomer, "CustomerId", (lang == "ar" ? "NAMEA" : "NAMEE"), (lang == "ar" ? "اختر العميل" : "Select customer"));
            }
        }
        if (compcode == 4) {
    ...
    function btnUpdate_onclick() {
        ...
        SysSession.CurrentEnvironment.I_Control[0].IvoiceDateEditable == true ? $('#txtInvoiceDate').removeAttr("disabled") : $('#txtInvoiceDate').attr("disabled", "disabled");
        var CustID = Number(ddlInvoiceCustomer.value);
        var Cust = CustomerDetails.filter(function (x) { return x.CustomerId == CustID; });
        filterCustomerDetails = CustomerDetails.filter(function (x) { return x.Isactive == true; });
    ...
    function BuildControls(cnt) {
        ...
        //var tr;
        // tr = '<select id="ddlFamily' + cnt + '" class="form-control"><option value="null">' + (lang == "ar" ? "النوع" : "Type") + '</option></select>';
        /*Old Table data*/
        //html = '<div id= "No_Row' + cnt + '" class="container-fluid style_border" > <div class="col-lg-12 col-md-12 col-sm-12 col-xl-12 col-xs-12" > <div class="col-lg-12 col-md-12 col-sm-12 col-xl-12 col-xs-12" > ' +
        //   '<span id="btn_minus' + cnt + '" class="fa fa-minus-circle fontitm3SlsTrSalesManager display_none"></span>' +
    ...
    function ComputeTotals() {
        ...
        var _loop_2 = function (i) {
            flagvalue = $("#txt_StatusFlag" + i).val();
            if (flagvalue != "d" && flagvalue != "m") {
                var CatID_1 = Number($('option:selected', $('#ddlFamily' + i)).attr('data-CatID'));
    ...
    function success_insert() {
        ...
        if (InvoiceStatisticsModel.length) {
            txtItemCount.value = InvoiceStatisticsModel[0].Line_Count.RoundToSt(2);
            txtPackageCount.value = InvoiceStatisticsModel[0].Tot_Qty.RoundToSt(2);
            txtTotal.value = InvoiceStatisticsModel[0].TotalAmount.RoundToSt(2);
            txtTax.value = InvoiceStatisticsModel[0].VatAmount.RoundToSt(2);
            txtNet.value = InvoiceStatisticsModel[0].NetAfterVat == null ? '' : InvoiceStatisticsModel[0].NetAfterVat.RoundToSt(2);
            txtCommission.value = InvoiceStatisticsModel[0].CommitionAmount.RoundToSt(2);
