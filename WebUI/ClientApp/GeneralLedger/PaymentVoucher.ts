...
namespace PaymentVoucher {
    ...
    var SysSession: SystemSession = GetSystemSession(Modules.PaymentVoucher);
    var compcode: Number;
    ...
    function BuildControls(cnt: number) {
        ...
        $('#btnSearchAcc' + cnt).click(function (e) {
            let sys: SystemTools = new SystemTools();
            if (VoucherCCDtType == 1) {
                sys.FindKey(Modules.PaymentVoucher, "btnAccountSearch", "COMP_CODE=" + compcode + "and ACC_ACTIVE = 1 and DETAIL =1 and CCDT_TYPE IS NOT NULL and  CCDT_TYPE != ''", () => {
                    let id = SearchGrid.SearchDataGrid.SelectedKey
                ...
                sys.FindKey(Modules.PaymentVoucher, "btnAccountSearch", "COMP_CODE=" + compcode + "and ACC_ACTIVE = 1 and DETAIL =1 ", () => {
                    let id = SearchGrid.SearchDataGrid.SelectedKey
                    $('#txtAccNumber' + cnt).val(id);
                    if (GetAccByCode(id)) {
                        $('#txtAccName' + cnt).val((lang == "ar" ? AccountDetails.ACC_DESCA : AccountDetails.ACC_DESCL));
        ...
        if (showFlag == true) {
            ...
            if (JournalDetailModelFiltered[cnt].CREDIT != null) $('#txtCredit' + cnt).val(JournalDetailModelFiltered[cnt].DEBIT.toString());
        ...
        if (TempshowFlag == true) {
            ...
            if (TemplateDetailModelFiltered[cnt].CREDIT != null) $('#txtCredit' + cnt).val(TemplateDetailModelFiltered[cnt].DEBIT.toString());
